...
async function generatePage() {
    try {
        let res = await exec(`ng g m pages/${folder}${pageName}-page --routing`);
        console.log(res.stdout);
        res = await exec(`ng g c pages/${folder}${pageName}-page`);
        console.log(res.stdout);
        const title = pageName.split('-').map(s => s.slice(0,1).toUpperCase() + s.slice(1)).join('');
        const componentName =  title + 'PageComponent';
        let routingFileText = fs.readFileSync(`src/app/pages/${folder}${pageName}-page/${pageName}-page-routing.module.ts`, 'utf-8');
        ...
        fs.writeFileSync(`src/app/pages/${folder}${pageName}-page/${pageName}-page-routing.module.ts`, routingFileText, 'utf-8');
        let navRoute =
            `path: '${pageName}',` +
            `loadChildren: () =>
                import('./pages/${folder}${pageName}-page/${pageName}-page.module').then(
